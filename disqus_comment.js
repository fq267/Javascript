					...
					setTimeout(function() {
						var operationOnLoadMoreCommentsIntervalId =  setInterval(function() {
							...
							if (checkByAsynMarkValue(asynMark, getCurrentAsynMark())) {
								var loadMoreEle =  $("iframe[src*='http://disqus.com/embed/comments/']").contents().find("a[data-action=more-posts]")[0];
								setTimeout(function() {
									if (loadMoreEle) {
...
function getCurrentAsynMark() {
	...
	var commentAsynMark = $("iframe[src*='http://disqus.com/embed/comments/']").contents().find('#post-list .post').length;
...
function operationOnLoadMoreComments(loadMoreEle, displayEle, operationOnLoadMoreCommentsIntervalId) {
	...
	if (shouldLoadMore(loadMoreEle, displayEle)) {
		if (loadCount < 4) {
			...
			var commentsDoc = $("iframe[src*='http://disqus.com/embed/comments/']")[0];
